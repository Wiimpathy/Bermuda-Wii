...
#ifdef GEKKO
		playVideoLogo("DATA/LOGO.AVI");
		playVideoIntro("DATA/INTRO.AVI");
#else
...
#ifdef GEKKO
void Game::playVideoLogo(const char *name) {
	char *filePath = (char *)malloc(strlen(_dataPath) + 1 + strlen(name) + 1);
	if (filePath) {
		sprintf(filePath, "%s/%s", _dataPath, name);
		File f;
		if (f.open(filePath)) {
			...
			_mixer->close();
			_mixer->open();
			strcpy(_musicName, "..\\MUSIC\\logo.ogg");
			playMusic(_musicName);
			AVI_Player player(_stub);
			player.play(&f);
			_mixer->close();
...
void Game::playVideoIntro(const char *name) {
	char *filePath = (char *)malloc(strlen(_dataPath) + 1 + strlen(name) + 1);
	if (filePath) {
		sprintf(filePath, "%s/%s", _dataPath, name);
		File f;
		if (f.open(filePath)) {
			...
			_mixer->close();
			_mixer->open();
			strcpy(_musicName, "..\\MUSIC\\intro.ogg");
			playMusic(_musicName);
			AVI_Player player(_stub);
			player.play(&f);
			_mixer->close();
...
void Game::playVideoFinal(const char *name) {
	char *filePath = (char *)malloc(strlen(_dataPath) + 1 + strlen(name) + 1);
	if (filePath) {
		sprintf(filePath, "%s/%s", _dataPath, name);
		File f;
		if (f.open(filePath)) {
			...
			_mixer->close();
			_mixer->open();
			strcpy(_musicName, "..\\MUSIC\\final.ogg");
			playMusic(_musicName);
			AVI_Player player(_stub);
			player.play(&f);
			_mixer->close();
